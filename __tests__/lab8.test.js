describe("Basic user flow for Website", () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const root = await items[0].getProperty("shadowRoot");
    // Once you have the button, you can click it and check the innerText property of the button.
    const addToCartBtn = await root.$("button");
  ...
  it("Checking number of items in cart on screen", async () => {
    ...
    for (let i = 1; i < items.length; i++) {
      const root = await items[i].getProperty("shadowRoot");
      const addToCartBtn = await root.$("button");
      await addToCartBtn.click();
    }
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    await page.reload();
    const items = await page.$$("product-item");
    for (const prodItem of items) {
      const root = await prodItem.getProperty("shadowRoot");
      const removeBtn = await root.$("button");
      const innerText = await removeBtn.getProperty("innerText");
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (const product of productItems) {
      const root = await product.getProperty("shadowRoot");
    ...
    const count = await page.$("#cart-count");
    const innerTextCart = await count.getProperty("innerText");
    const cartValue = await innerTextCart.jsonValue();
